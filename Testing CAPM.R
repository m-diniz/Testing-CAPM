...
# Simulate average returns of the firms
mu_ret <- rnorm(i, 0, 0.01)

# Simulate standart deviations of returns
...
# Set risk free rate constant, in this example
...
index <- match(sd_rets[which.min(sd_rets)], sd_rets)
# column 54 will be the market portfolio, because is expected low variance from market portfolio

# Step (1): Run individual regressions -----------------------------------
# We'll regress individual returns agains portfolio returns to get betas
# Note 1: regression of returns in EXCESS
betas <- apply(my_data_excess, 2, function(x){lm(x ~ my_data_excess[,index])$coefficients[2]})



# Step (2): Run betas against average returns -----------------------------
# Note 2: regression of ACTUAL returns, not EXCESS returns
